...
POC TARGET:
Airflow
    ↓  (triggers Glue directly using GlueJobOperator)
AWS Glue
    ↓  (calls SAP OData API directly — no SnapLogic)
    ↓  (extracts + transforms in one job)
AWS S3 (same as before)
    ↓
AWS Glue Transform (same existing templates — no change)
    ↓
Snowflake (separate POC schema: GLUE_POC_DB.SAPS4_CORE)
    ↓
ThoughtSpot / Power BI (same as before)
```

**What changes:**
SnapLogic is REMOVED. Glue now calls SAP OData API directly.

**What stays exactly the same:**
S3 structure, Glue transform jobs, Snowflake, Airflow, ThoughtSpot.
...
| Configuration | MySQL RDS INPUT_ENTITY_LIST | Snowflake GLUE_ENTITY_CONFIG |
...
│                                                                          │
│  s3://<bucket>/glue_poc/sap/BillingDocument/full/<date>/                │
│  s3://<bucket>/glue_poc/sap/BillingDocument/delta/<date>/               │
│  s3://<bucket>/glue_poc/sap/BillingDocumentItem/full/<date>/            │
│  s3://<bucket>/glue_poc/sap/BillingDocumentItem/delta/<date>/           │
...
2. Create these folders (click "Create folder" for each path):

   glue_poc/
   glue_poc/sap/
   glue_poc/sap/BillingDocument/
   glue_poc/sap/BillingDocument/full/
   glue_poc/sap/BillingDocument/delta/
   glue_poc/sap/BillingDocumentItem/
   glue_poc/sap/BillingDocumentItem/full/
   glue_poc/sap/BillingDocumentItem/delta/
   glue-scripts/          ← upload Python scripts here
   glue-temp/             ← Glue uses this for temp files

Final S3 structure:
<your-bucket>/
├── glue_poc/sap/BillingDocument/full/<date>/    ← full load Parquet
├── glue_poc/sap/BillingDocument/delta/<date>/   ← delta load Parquet
├── glue_poc/sap/BillingDocumentItem/full/<date>/
├── glue_poc/sap/BillingDocumentItem/delta/<date>/
├── glue-scripts/                                 ← Python job files
└── glue-temp/                                    ← Glue temp directory
```
...
## 8.2 GLUE_ENTITY_CONFIG Control Table

This table is the brain of the system.
Every Glue job reads from here to know: what to extract, where to write,
what watermark to use, what page size to use.
This replaces SnapLogic's MySQL INPUT_ENTITY_LIST.

```sql
USE DATABASE PRD_ARCH_SUPPORT_DB;
USE SCHEMA GLUE_POC;

CREATE TABLE IF NOT EXISTS GLUE_ENTITY_CONFIG (
    CONFIG_ID          NUMBER AUTOINCREMENT PRIMARY KEY,

    SRC_SYS_NM         VARCHAR(20)  NOT NULL,
    -- Source system identifier. Values: SAPS4, SFDC
    -- All SAP tables use: SAPS4

    SRC_API_NM         VARCHAR(200) NOT NULL,
    -- OData service path (the API endpoint path)
    -- e.g. /sap/opu/odata/sap/API_BILLING_DOCUMENT_SRV
    -- This is appended to base_url from Secrets Manager

    SRC_OBJ_NM         VARCHAR(100) NOT NULL,
    -- OData entity set name (what comes after the service path)
    -- e.g. A_BillingDocument
    -- Full URL = base_url + SRC_API_NM + "/" + SRC_OBJ_NM

    RELATIVE_PATH      VARCHAR(100),
    -- Friendly name used as S3 subfolder and for lookups
    -- e.g. BillingDocument, BillingDocumentItem

    LOAD_TYP           VARCHAR(1)   NOT NULL,
    -- D = Delta load (use watermark filter)
    -- F = Full load (no date filter — used for master data)

    REQUIRED_FLAG      VARCHAR(1)   DEFAULT 'Y',
    -- Y = Active (will be extracted)
    -- N = Inactive/skip (won't be extracted)

    CDC_WATERMARK_COL  VARCHAR(100),
    -- OData field name used for delta filtering
    -- e.g. LastChangeDateTime
    -- Used in: $filter=LastChangeDateTime gt datetime'<watermark>'

    LAST_WATERMARK     TIMESTAMP_NTZ,
    -- Last successful extraction timestamp
    -- Glue updates this after every successful run
    -- Next run extracts only records changed AFTER this timestamp

    BATCH_SIZE         NUMBER       DEFAULT 1000,
    -- OData $top page size — how many rows per API call
    -- Recommended: 1000 (safe for most SAP systems)
    -- Can increase to 5000 if SAP team approves

    STATIC_PARAMS      VARCHAR(500),
    -- Additional static OData $filter conditions
    -- e.g. "CompanyCode eq '1000'"
    -- Appended to the filter string with AND

    S3_PATH            VARCHAR(200),
    -- S3 path prefix for this entity's output
    -- e.g. glue_poc/sap/BillingDocument

    CREATED_AT         TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(),
    UPDATED_AT         TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP()
);

COMMENT ON TABLE GLUE_ENTITY_CONFIG IS
'Control table for all Glue POC extractions.
 Stores OData config, watermarks, and S3 output paths.
 Aligned with SnapLogic INPUT_ENTITY_LIST for consistency.
 From Solution Design v2, Section 7.1.
 Location: PRD_ARCH_SUPPORT_DB.GLUE_POC.GLUE_ENTITY_CONFIG';
```
...
```sql
-- Insert config for BillingDocument (Parent entity)
INSERT INTO PRD_ARCH_SUPPORT_DB.GLUE_POC.GLUE_ENTITY_CONFIG
...
VALUES (
    'SAPS4',
    '/sap/opu/odata/sap/API_BILLING_DOCUMENT_SRV',   -- OData service
    'A_BillingDocument',                               -- Entity set
    'BillingDocument',                                 -- Friendly name / S3 folder
    'D',                                               -- Delta load
    'Y',                                               -- Active
    'LastChangeDateTime',                              -- Watermark OData field
    DATEADD('day', -30, CURRENT_TIMESTAMP()),          -- Start 30 days back for POC
    1000,                                              -- Page size ($top)
    NULL,                                              -- No static filters
    'glue_poc/sap/BillingDocument'                    -- S3 output path prefix
);

-- Insert config for BillingDocumentItem (Child entity)
INSERT INTO PRD_ARCH_SUPPORT_DB.GLUE_POC.GLUE_ENTITY_CONFIG
    (SRC_SYS_NM, SRC_API_NM, SRC_OBJ_NM, RELATIVE_PATH,
     LOAD_TYP, REQUIRED_FLAG, CDC_WATERMARK_COL,
     LAST_WATERMARK, BATCH_SIZE, STATIC_PARAMS, S3_PATH)
...
-- Verify both records
SELECT CONFIG_ID, SRC_OBJ_NM, LOAD_TYP,
       TO_CHAR(LAST_WATERMARK, 'YYYY-MM-DD HH24:MI:SS') AS watermark_start,
       BATCH_SIZE, S3_PATH
FROM PRD_ARCH_SUPPORT_DB.GLUE_POC.GLUE_ENTITY_CONFIG
ORDER BY CONFIG_ID;
-- Expected: 2 rows. BillingDocument first, BillingDocumentItem second.
```
...
Task flow:
  start
    → extract_billingdocument       (Glue job — parent)
    → extract_billingdocumentitem   (Glue job — child, only after parent)
...
========================================
Orchestrates:
  1. BillingDocument OData extraction (parent)
  2. BillingDocumentItem OData extraction (child)
  3. Row count validation in Snowflake
...
# Job parameters passed to both Glue jobs
GLUE_JOB_PARAMS = {
    '--sap_secret_name':       'tcpl/sap/odata-credentials',
    '--snowflake_secret_name': 'tcpl/snowflake/glue-poc-credentials',
...
with DAG(
    ...
    description='Glue POC — SAP BillingDocument + BillingDocumentItem OData extraction',
) as dag:
    ...
    extract_billing_document = GlueJobOperator(
        ...
        job_name='glue-poc-billingdocument-odata-extract',
        script_args={
            **GLUE_JOB_PARAMS,
            '--entity_name': 'BillingDocument',
        },
    ...
    extract_billing_doc_item = GlueJobOperator(
        ...
        job_name='glue-poc-billingdocumentitem-odata-extract',
        script_args={
            **GLUE_JOB_PARAMS,
            '--entity_name': 'BillingDocumentItem',
        },
    ...
    validate = SnowflakeOperator(
        ...
        sql="""
            SELECT 'BILLINGDOCUMENT'     AS table_name,
                   COUNT(*)              AS rows_loaded,
                   MAX(EXTRACTED_AT)     AS last_load
            FROM GLUE_POC_DB.SAPS4_CORE.BILLINGDOCUMENT
            UNION ALL
            SELECT 'BILLINGDOCUMENTITEM' AS table_name,
                   COUNT(*)              AS rows_loaded,
                   MAX(EXTRACTED_AT)     AS last_load
    ...
    # CRITICAL: parent → child → validate
    # VBRK must always complete before VBRP
    start >> extract_billing_document >> extract_billing_doc_item >> validate >> end
```
...
| Config | MySQL RDS INPUT_ENTITY_LIST | Snowflake GLUE_ENTITY_CONFIG |
...
| GLUE_ENTITY_CONFIG | Snowflake control table — stores OData config and watermarks per entity |
