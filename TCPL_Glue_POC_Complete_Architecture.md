...
CURRENT STATE (what exists today — using SnapLogic):
──────────────────────────────────────────────────────
  SAP S/4HANA  ──┐
                  ├──► SnapLogic ──► S3 ──► Glue Transform ──► Snowflake
  Salesforce   ──┘    (extractor)         (already exists)

  Problem: SnapLogic is expensive. Charges per pipeline/connector.
           At TCPL scale (250+ pipelines) = very high cost.
...
──────────────────────────────────────────────────
  ...
  Goal: Replace SnapLogic with Glue for extraction layer.
        Prove data accuracy = 100% match.
        If POC passes → migrate all 250+ SnapLogic pipelines.
...
└─────────────────────────────────────────────────────────────────────────────┘
                          ...
                          │  2. INSERT INTO GLUE_AUDIT_LOG (status, count, etc)
                          │
...
│                                                                             │
│  Test B — MINUS Query (records missing in Glue):                           │
│    SELECT cols FROM SAPS4_DB...                                             │
│    MINUS                                                                    │
│    SELECT cols FROM GLUE_POC_DB...    → 0 rows  ✅                         │
│                                                                             │
│  Test C — MINUS Query (extra records in Glue):                             │
...
─────────────────────────────────────
  ...
  □ Click Run → watch CloudWatch logs
  □ Expected output:
...
───────────────────────────────────────────────────────
  □ Create Glue job for Table 1 in Console:
      Name:       tcpl-glue-poc-sfdc-extract  (or sap)
      Script:     s3://.../glue_poc_sfdc_extract.py
      Workers:    10 × G.1X
...
────────────────────────────────────────
  ...
  □ Check GLUE_AUDIT_LOG in Snowflake:
      ...
      → Should show 1 row: SecondarySalesLineItem | FULL | 122600000 | SUCCESS
...
─────────────────────────────────────────────────
  ...
  □ Validation E — Delta window check:
      SELECT 'SnapLogic delta', COUNT(*) FROM SAPS4_DB...
      WHERE LASTCHANGEDATETIME BETWEEN '2024-03-03' AND '2024-03-04'
      UNION ALL
      SELECT 'Glue delta', COUNT(*) FROM GLUE_POC_DB...
...
──────────────────────────────────
  □ Fill in validation report:
      ...
      SecSalesLineItem | 122.6M    | 122.6M    | ✅ YES
      ...all 12 tables

  □ Note performance timings:
      BillingDocument (5.4M) → Glue: 6 min vs SnapLogic: 8 min → 25% faster
      SecSalesLineItem (122M) → Glue: 38 min vs SnapLogic: 45 min → 16% faster
...
TCPLGluePOCRole must have:
  ...
  ├── S3 Read/Write on s3://tcpl-datalake/glue_poc/*
  ├── S3 Read on s3://tcpl-datalake/scripts/glue_poc/*
  ├── Secrets Manager GetSecretValue on tcpl/glue_poc/*
  └── CloudWatch Logs write on /aws-glue/*
...
ISSUE                          SYMPTOM                    FIX
─────────────────────────────────────────────────────────────────────────────
SAP firewall not open          Connection timeout          Escalate daily to infra.
                                                           Build SAP code meanwhile.
                                                           Switch to SFDC first.

SFDC INVALID_LOGIN             OAuth 400 error            Add security_token to secret.
                                                           Ask SFDC admin to set
                                                           IP Relaxation on Connected App.

SAP pagination stops early     Only 5000 rows extracted   Check response for nextLink.
                               even with more data        Update: data.get("@odata.nextLink")
                                                          or data.get("d",{}).get("__next")

Glue OOM on 122M SFDC table   Job killed, no output       Write page-by-page to S3.
                                                           Increase DPUs 10 → 20.

Watermark not updated          Data in S3 but LAST_        Wrap update_watermark() in
                               WATERMARK unchanged         try-finally. Manually update:
                                                           UPDATE GLUE_ENTITY_CONFIG
                                                           SET LAST_WATERMARK = '2024-03-04'
                                                           WHERE SRC_OBJ_NM = 'BillingDocument';

MINUS returns rows even        Row counts match but        Cast both to same type.
with matching row counts       data differs               Check timestamp/numeric format.
                                                          CAST(col AS VARCHAR) in MINUS.
